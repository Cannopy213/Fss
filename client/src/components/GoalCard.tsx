...
export function GoalCard({ goal, onContribute, onWithdraw, onViewDetails, userAddress }: GoalCardProps) {
  ...
  const isExpired = new Date() > goal.deadline;
  const daysRemaining = Math.ceil((goal.deadline.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
  
  const getStatusBadge = () => {
    if (goal.isAchieved) {
      return <Badge className="bg-success bg-opacity-10 text-success">مُحقق</Badge>;
    }
    if (isExpired) {
      return <Badge variant="destructive">منتهي الصلاحية</Badge>;
    }
    if (daysRemaining <= 7) {
      return <Badge className="bg-warning bg-opacity-10 text-warning">قريب الانتهاء</Badge>;
    }
    return <Badge className="bg-secondary bg-opacity-10 text-secondary">نشط</Badge>;
  };
      ...
      <CardContent className="p-6">
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-medium text-gray-600">التقدم</span>
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <p className="text-xs text-gray-500">المُوفر</p>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {daysRemaining > 0 && !goal.isAchieved && (
            <div className="flex items-center space-x-1 space-x-reverse">
              ...
              <span className="text-sm text-gray-600">{daysRemaining} يوم متبقي</span>
            </div>
          )}
        </div>

        {/* NFT Achievement Badge for completed goals */}
        {goal.isAchieved && (
          <div className="bg-gradient-to-r from-achievement to-purple-600 rounded-lg p-3 mb-4 text-white">
            <div className="flex items-center space-x-2 space-x-reverse">
              <Trophy className="w-5 h-5" />
              <div>
                <p className="text-sm font-medium">حصلت على مكافأة NFT!</p>
                <p className="text-xs opacity-90">شارة إنجاز التوفير الذهبية</p>
              </div>
            </div>
          </div>
        )}

        {/* Urgent contribution warning */}
        {!goal.isAchieved && daysRemaining <= 7 && daysRemaining > 0 && (
          <div className="bg-warning bg-opacity-10 rounded-lg p-3 mb-4">
            <div className="flex items-center space-x-2 space-x-reverse">
              <Clock className="w-4 h-4 text-warning" />
              <p className="text-sm text-warning font-medium">
                باقي {daysRemaining} أيام على انتهاء المهلة
              </p>
            </div>
          </div>
        )}
          ...
          {goal.isAchieved ? (
            ...
            <>
              <Button 
                ...
                disabled={isExpired}
              >
                {daysRemaining <= 7 ? 'مساهمة عاجلة' : 'مساهمة'}
              </Button>
              <Button 
                onClick={() => onViewDetails(goal)}
                variant="outline"
